...
% ADD OR EXCLUDE TERMS HERE 
modelspec(1).cvfunc = @(t1,t2,dt1t2,thetas,ad,fp1fp2)  cvfunc.G(dt1t2,thetas(1:2)) + cvfunc.L(dt1t2,ad,thetas(6:8)) + cvfunc.W(dt1t2,ad,thetas(9)) + cvfunc.RN(t1,t2,dt1t2,ad,thetas(3:5)); 
   % + cvfunc.RL(t1,t2,dt1t2,ad,thetas(3:4)) 
     

modelspec(1).dcvfunc =  @(t1,t2,dt1t2,thetas,ad,fp1fp2) dcvfunc.G(t1,t2,dt1t2,thetas(1:2)) + dcvfunc.L(t1,t2,dt1t2,ad,thetas(6:8)) + dcvfunc.RN(t1,t2,dt1t2,ad,thetas(3:5));
  % + dcvfunc.RL(t1,t2,dt1t2,ad,thetas(3:4)) 
    

modelspec(1).ddcvfunc =  @(t1,t2,dt1t2,thetas,ad,fp1fp2) ddcvfunc.G(dt1t2,thetas(1:2)) + ddcvfunc.L(dt1t2,ad,thetas(6:8)) + ddcvfunc.RN(dt1t2,ad,thetas(3:5));
  % + ddcvfunc.RL(t1,t2,dt1t2,ad,thetas(3:4)) 
...
tluTGG = [
    ...
    500 1 30000    % 1 Matern global amplitude 
    100 1 30000  % 2 Temporal parameter
 
    1 0.01 30000      %  3 Linear amplitude
    1 0.01 12         %  4 Geographic length scale

    100 0.01 30000    % 5 Matern regional amplitude
    100 1 30000   % 6 Temporal parameter
    2 1 12         % 7 Geographic length scale
    
    100 0.01 30000    % 8 Matern local amplitude
    100 1 30000     % 9 Temporal parameters
    0.01 0.001 5     % 10 Geographic length scale
 
    100 0.1 10e5    % 11 white noise
         ];

%% CHANGE THE TERMS TO INCLUDE IN THE MODEL HERE
modelspec(1).thet0=tluTGG(:,1)';
modelspec(1).lb = tluTGG(:,2)';
modelspec(1).ub = tluTGG(:,3)';
% LIST EXCLUDED TERMS HERE
modelspec(1).subfixed=[3 4];

modelspec(1).sublength=[7 10]; 
modelspec(1).subamp = [1 5 8 11];
