function[]=PlotRSLDecompositionSummarised(runs_to_plot,RSLmaxage, datasets,f2s,sd2s,V2s,testlocs,legendrows,subplotcolno,filename,imageres)
...
for k = 1:n_plot
    ...
    % row index for first site (to plot for global component as it is the
    % same for all sites)
    index = find(testlocs.reg==31000);
    ...
    hold on
    h=plot(t,RSLmean,'Color',col,'DisplayName',sprintf('Model %d',m)); % RSL mean line
    ...
    for i=1:length(testlocs.names)-1 % loop for all local sites (remove last row as that is for Barbados)
...
for k = 1:n_plot
    ...
    % Plot RSL rates
    subplot(subplotrowno,subplotcolno,1);
    h=plot(difftimes,df2s,'Color',col,'Display',sprintf('Model %d',m)); % RSL rate mean
    legend_all=[legend_all; h];
    hold on
