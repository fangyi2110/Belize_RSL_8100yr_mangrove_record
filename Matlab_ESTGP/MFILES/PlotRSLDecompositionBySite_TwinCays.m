...
subplotrowno = ceil((length(testlocs.names)+1)/subplotcolno);
...
for k = 1:n_plot
    ...
    % noise; use {2} for global component only and {3} for local component only
...
for k = 1:n_plot
    ...
    xlabel('Age (BP)')
    set(gca, 'XDir', 'reverse');% Reverse x axes
    title(sprintf('Mean RSL rate for Twin Cays'))
