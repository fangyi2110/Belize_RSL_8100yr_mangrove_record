function [testnames, rate_samples_all] = makeplot_slrate(dataset,f2s_shifted,sd2s,V2s,testloc,labl,doplots,difftimestep,xlim0,xrange,meanSL,plot_dat,GIAtime,GIAsite,GIArsl6G,GIArsl7G,yl)
...
elseif xlim0==3
 ...
 elseif xlim0==42
 maxdistfrom=1; % Maximum angular distance (in degrees) allowed between test location and dataset point
    % for inclusion in the plot or analysis. Here set to 1 degree (roughly ~111 km at equator).
 maxerror=5000; % Maximum vertical uncertainty/error (in mm) allowed in the data points to be considered.
    % Data with higher uncertainty might be excluded.
 difftimestep=100; % Temporal spacing (in years) between time steps for computing and plotting rate estimates.
 xl=xrange; % x-axis limits for the plot (in years CE)
 % define y axis limits RSL heightaccording to the data 
 pred_min = min(f2s_shifted(:) - 2*sd2s(:)); 
 pred_max = max(f2s_shifted(:) + 2*sd2s(:));
 margin = 0.05 * (pred_max - pred_min);  % Padding
 yl = [pred_min - margin, pred_max + margin]; % y-axis limits for the RSL height plots (in mm). 
end
...
fid=fopen([labl ' - Site RSL and Rates.tsv'],'w');
fclose(fid);
...
for i=1:size(testsites,1)
    ...
    if difftimestep>0
        ...
        for n=1:size(f2s,2)
            df2s(:,n)=Mdiff*f2s(:,n); % calculate mean rates as differential of RSL mean function
            dV2s(:,:,n)=Mdiff*V2s(:,:,n)*Mdiff'; % calculate rate covariance matrix
            dsd2s(:,n)=sqrt(diag(dV2s(:,:,n))); % calculate stdev of rates from diagonal of covariance matrix
        end

        % Sample posterior rates
        nsamps = 1000;
        V_r = dV2s;
        mu_r=df2s;
        % Ensure perfect symmetry in covariance matrix (minor numerical asymmetry can appear)
        V_r = 0.5*(V_r + V_r.');
        % Add a very small "jitter" (stabilizing term) to the diagonal
        jitter = 1e-10 * trace(V_r) / size(V_r,1);
        V_r = V_r + jitter * eye(size(V_r,1));
        % Try Cholesky decomposition (V_r = L*L'), where L is lower-triangular
        [L,p] = chol(V_r,'lower');
        if p ~= 0
            % If Cholesky fails, matrix is still not positive definite.
            % Increase jitter by 100× and retry.
            jitter = 1e-8 * trace(V_r) / size(V_r,1);
            V_r = V_r + (jitter) * eye(size(V_r,1));
            L = chol(V_r,'lower');  % may still error if extremely ill-conditioned
        end
        % draw samples
        Z = randn(size(V_r,1), nsamps);
        rate_samples = repmat(mu_r,1,nsamps) + L * Z;   % nRates x nsamps

        % Organise rate samples to indicate site name
        rate_samples_all{i} = struct( ...
            'site', testnames{i}, ...
            'times', difftimes(subA2), ...
            'samples', rate_samples(subA2, :));
        ...
        % writes RSL, rates and SDs to tsv file
        fid = fopen([labl ' - Site RSL and Rates.tsv'], 'a');
        fprintf(fid,['\n \n' testnames{i} '\t']);
        fprintf(fid,'%0.2f\t',testloc.sites(i,1)); 
        fprintf(fid,'%0.2f',testloc.sites(i,2));

        if i == 1
            fprintf(fid,'\n Test Times \t');
            for ii = 1:length(testX(subA,3))
                fprintf(fid,'%0.1f\t',testX(subA(ii),3));
            end
        end
        ...
        for ii = 1:length(testX(subA,3))
            fprintf(fid,'%0.2f\t',f2s(subA(ii))/1000);
        end

        fprintf(fid,'\n RSL SDs \t');
        
        for ii = 1:length(testX(subA,3))
            fprintf(fid,'%0.2f\t',sd2s(subA(ii))/1000);
        end

        % now the rates
        if i == 1
            fprintf(fid,'\n Rate Times \t');
            for ii = 1:length(difftimes(subA2))
                fprintf(fid,'%0.1f\t',difftimes(subA2(ii)));
            end
        end
        ...
        for ii = 1:length(difftimes(subA2)) 
            fprintf(fid,'%0.2f\t',df2s(subA2(ii)));
        end
        fprintf(fid,'\n Rate SDs \t');
        
        for ii = 1:length(difftimes(subA2))
            fprintf(fid,'%0.2f\t',dsd2s(subA2(ii)));
        end
        fclose(fid);          
