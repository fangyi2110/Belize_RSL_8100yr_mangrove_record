...
iii=1;
    ...
    noiseMasks(2,[1 3 8 11]) = 0;   % Regional matern only
    noiseMasks(3,[1 5 8 11]) = 0;   % Regional linear only
    noiseMasks(4,[3 5 8 11]) = 0;   % Global only
    noiseMasks(5,[1 3 5 11]) = 0;   % Local

    %new addition
    %noiseMasks(5,[1 5 8 11]) = 0;   % Linear regional only

    %combine signals
    noiseMasks(6,[8 11]) = 0;   % All but local and white noise
...
lab=[{'Full'} {'Regional matern'} {'Regional Linear'} {'Global'} {'Local'} {'Local removed'}];
