...
%% change map years here
trainsub = find(wdataset.limiting==0);
cc=find(wdataset.istg==2);
tg=find(wdataset.istg==1);
di=find(wdataset.istg==0);
%firstyears=[-10000:4000:0];% 1000:200:1800 1900 ]; 
%lastyears=[-6000:4000:2000];% 1100:200:2000 2000];
firstyears=[-10000:1000:1000];% 1000:200:1800 1900 ]; 
lastyears=[-9000:1000:2000];% 1100:200:2000 2000];
...
for i=1:length(firstyears)
    ...
    hs1=scatterm(FLAT(:),FLONG(:),300,mapped(:),'filled','marker','s');
...
axis tight;
hcb=colorbar;
%CHANGE COLOR BAR SCALE HERE
% caxis([min(mapped(:))-0.05 max(mapped(:))+0.05]);
% caxis([-2 2]);

if i==1 % 12 to 11 ka
    caxis([-31 2]);
elseif i==2 % 11 to 10 ka
    caxis([-31 2]);
elseif i==3 % 10 to 9 ka
    caxis([-31 2]);
elseif i==4 % 9 to 8 ka
    caxis([-31 2]);
else % after 8 ka
    caxis([-2 2]);
end
...
box on;
    ...
    hs1=scatterm(FLAT(:),FLONG(:),250,mapRate(:),'filled','marker','s');
     ...
     hcb=colorbar;
% CHANGE RATE SCALE HERE
     caxis([min(mapRate(:))-0.05 max(mapRate(:))+0.05]);
     %if i==1
     %    caxis([5.4 7.4]);
     %elseif i==2
     %   caxis([2 3.8]);
