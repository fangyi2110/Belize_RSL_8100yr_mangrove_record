...
firstyears=[-3000:500:1500];
lastyears=[-2500:500:2000];
...
for i=1:length(firstyears)
    ...
    [fslopeFs,sdslopeF,fsF,sdsF,~,~,~,~,passderivs,invcv,fmeanFs,fsdF] = RegressRateField_ea(wdataset,wmodelspec,thetTGG{jj},noiseMasks(6,:),Flat,Flong,firstyears(i),lastyears(i),trainsub,[],[],[]);
    [priorslope,sdpriorslope,priorfsF,priorsdsF] = RegressRateField_ea(nulldataset,wmodelspec,thetTGG{jj},noiseMasks(6,:),Flat,Flong,firstyears(i),lastyears(i),trainsub,[],[],[]);
    ...
    box on;   
    axis tight;
    hcb=colormap(brewermap([],"RdBu"));
    %caxis([min(mapped_Reg(:))-0.05 max(mapped_Reg(:))+0.05]);
    caxis([-1 1]);
    colorbar;

    title({['NoLocal_Signal' num2str(firstyears(i)) ' to ' num2str(lastyears(i)) ' CE']});
    pdfwrite(['NoLocal_no_land_' num2str(firstyears(i)) '_to_' num2str(lastyears(i))]);

    scatterm(lat_site,long_site,3*ndat,'MarkerEdgeColor','k','MarkerFaceColor','w');
    pdfwrite(['NoLocal_no_land_data_' num2str(firstyears(i)) '_to_' num2str(lastyears(i))]);
    box on;
      ...
      pdfwrite(['NoLocal_RSL' labl '_' num2str(firstyears(i)) '_to_' num2str(lastyears(i)) '_no_data']);
end
